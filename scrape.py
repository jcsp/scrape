...
class LockdepReason(AssertionReason):
    ...
    def could_be(cls, job):
        ...
        if "common/lockdep" in job.get_assertion():
            return True
        else:
            return False
...
class DeadReason(Reason):
    ...
    def match(self, job):
        ...
        ratio = difflib.SequenceMatcher(None, self.last_tlog_line, job.get_last_tlog_line()).ratio()
        return ratio > 0.5
...
class TimeoutReason(Reason):
    ...
    def match(self, job):
        if self.get_timeout(job) == (self.timeout, self.command):
            return True
        else:
            return False
...
class ValgrindReason(Reason):
    ...
    def _get_service_types(self, job):
        ...
        for line in grep(os.path.join(job.path, "teuthology.log"), "</kind> in s"):
